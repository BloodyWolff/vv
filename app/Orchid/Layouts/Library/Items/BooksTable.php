...
class BooksTable extends Table
{
    ...
    protected function columns(): iterable
    {
        return [
            ...
            TD::make('year_of_publishing', 'Год издания'),
            TD::make('publishing', 'Издательство')
                -> render(fn (Book $book) => $book -> publishing -> name),
            TD::make('count_of_items', 'Количество экземпляров (учетные)'),
            TD::make('type_of_books', 'Тип экземпляра')
                -> render(fn (Book $book) => $book -> TypeOfBook -> name),
    ...
    /**
     * Сделали нормальное отображение издательства и типа экземпляра.
     * рендер помогает вычленять из модели, которая берет определенную информацию из таблицы
     */